      ...
      <form onSubmit={onSubmitHandler} className='border-2 bg-white/8 text-white border-gray-500 p-6 flex flex-col gap-6 rounded-lg shadow-lg'>
        ...
        </h2>
        {
          currState == "Sign up" && !isDataSubmitted &&(
            ...
            focus:outline-none' placeholder='Full Name' value={fullName} required/>
          )
        }
        { !isDataSubmitted && (
          <>
          <input onChange={(e)=>setEmail(e.target.value)} value={email} 
          type='email' placeholder='Emial Address' required className='p-2 border
        ...
        <div className='flex flex-col gap-2'>
          {currState === 'Sign up'?(
            <p className='text-sm text-gray-600 '>
              ...
              onClick={()=>{setCurrState("Login")}} className='font-medium text-violet-500 cursor-pointer'>Login here</span>
            </p>
          ):(
